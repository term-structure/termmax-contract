...
import {IGearingTokenV1Plus} from "./IGearingTokenV1Plus.sol";
import {GearingTokenEventsV1Plus} from "../events/GearingTokenEventsV1Plus.sol";
import {GearingTokenErrorsV1Plus} from "../errors/GearingTokenErrorsV1Plus.sol";
/**
 * @title TermMax Gearing Token
 * @author Term Structure Labs
 */

abstract contract AbstractGearingTokenV1Plus is
    OwnableUpgradeable,
    ...
    GearingTokenEvents
{
        ...
        initializer
    {
        ...
        emit GearingTokenEventsV1Plus.GearingTokenInitialized(msg.sender, name, symbol, initalParams);
    }
        ...
        onlyInitializing
    {
        ...
        if (config_.loanConfig.liquidationLtv > Constants.DECIMAL_BASE) {
            revert GearingTokenErrorsV1Plus.InvalidLiquidationLtv();
        }
    ...
    function augmentDebt(address caller, uint256 id, uint256 ftAmt) external virtual override nonReentrant onlyOwner {
        ...
        if (config.maturity <= block.timestamp) {
            revert GearingTokenErrorsV1Plus.GtIsExpired();
        }
    ...
    function merge(uint256[] memory ids) external virtual nonReentrant returns (uint256 newId) {
        if (ids.length == 0) {
            revert GearingTokenErrorsV1Plus.GtIdArrayIsEmpty();
        }
        GtConfig memory config = _config;
        if (config.maturity <= block.timestamp) {
            revert GearingTokenErrorsV1Plus.GtIsExpired();
    ...
    function repay(uint256 id, uint128 repayAmt, bool byDebtToken) external virtual override nonReentrant {
        ...
        emit GearingTokenEventsV1Plus.Repay(id, repayAmt, byDebtToken, repayAll);
    }
    ...
    function _flashRepay(
        ...
        emit GearingTokenEventsV1Plus.FlashRepay(id, msg.sender, repayAmt, byDebtToken, repayAll, removedCollateral);
        return repayAll;
    }
