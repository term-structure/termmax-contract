...
import {MarketConstantsV1Plus} from "./lib/MarketConstantsV1Plus.sol";
...
import {IMintableERC20V1Plus} from "./tokens/IMintableERC20V1Plus.sol";
import {ITermMaxOrderV1Plus} from "./ITermMaxOrderV1Plus.sol";

/**
 * @title TermMax Market V1Plus
 * @author Term Structure Labs
 */
contract TermMaxMarketV1Plus is
    ITermMaxMarket,
    ...
    MarketEvents
{
    ...
    using TransferUtilsV1Plus for IMintableERC20;
    ...
    function initialize(MarketInitialParams memory params) external virtual override initializer {
        ...
        name = StringUtil.contact(MarketConstantsV1Plus.PREFIX_MARKET, params.tokenName);
        ...
        returns (IMintableERC20 ft_, IMintableERC20 xt_, IGearingToken gt_)
    {
        ...
        ft_.initialize(
            MarketConstantsV1Plus.PREFIX_FT.contact(params.tokenName),
            MarketConstantsV1Plus.PREFIX_FT.contact(params.tokenSymbol),
            decimals
        );
        xt_.initialize(
            MarketConstantsV1Plus.PREFIX_XT.contact(params.tokenName),
            MarketConstantsV1Plus.PREFIX_XT.contact(params.tokenSymbol),
            decimals
        );
        gt_.initialize(
            MarketConstantsV1Plus.PREFIX_GT.contact(params.tokenName),
            MarketConstantsV1Plus.PREFIX_GT.contact(params.tokenSymbol),
            GtConfig(
                params.collateral,
                params.debtToken,
                ft_,
    ...
    /**
     * @inheritdoc ITermMaxMarketV1Plus
     */
    ...
    function _burn(address owner, address spender, address recipient, uint256 debtTokenAmt) internal {
        IMintableERC20V1Plus(address(ft)).burn(owner, spender, debtTokenAmt);
        IMintableERC20V1Plus(address(xt)).burn(owner, spender, debtTokenAmt);
    ...
    function _leverageByXt(
        ...
        IMintableERC20V1Plus(address(xt)).burn(xtOwner, msg.sender, xtAmt);
        ...
        returns (uint256 debtTokenAmt, bytes memory deliveryData)
    {
        ...
        if (ftReserve > 0) {
            IMintableERC20V1Plus(address(ft)).burn(address(this), address(this), ftReserve);
        }
        ...
        // Burn ft
        IMintableERC20V1Plus(address(ft)).burn(ftOwner, caller, ftAmount);
    ...
    function _createOrder(address maker, OrderConfig memory orderconfig) internal returns (ITermMaxOrder) {
        ...
        ITermMaxOrderV1Plus(order).initialize(maker, [ft, xt, debtToken], gt, orderconfig, _config);
