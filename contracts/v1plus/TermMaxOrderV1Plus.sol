...
import {OrderEventsV1Plus} from "./events/OrderEventsV1Plus.sol";
import {OrderErrorsV1Plus} from "./errors/OrderErrorsV1Plus.sol";

/**
 * @title TermMax Order V1Plus
 * @author Term Structure Labs
 */
contract TermMaxOrderV1Plus is
    ITermMaxOrder,
    ITermMaxOrderV1Plus,
    ...
    OrderEvents
{
    ...
    function initialize(
        ...
        revert OrderErrorsV1Plus.UseOrderInitializationFunctionV1Plus();
    }

    /**
     * @inheritdoc ITermMaxOrderV1Plus
     */
    function initialize(
        address maker_,
        ...
        emit OrderEventsV1Plus.OrderInitialized(maker_, address(market), orderConfig_);
    }
    ...
    function _sellToken(
        ...
        ITermMaxMarketV1Plus(address(market)).burn(address(this), recipient, netOut);
        return (netOut, feeAmt);
    }
    ...
    function _sellTokenForExactToken(
        ...
        ITermMaxMarketV1Plus(address(market)).burn(address(this), recipient, debtTokenAmtOut);
        return (netTokenIn, feeAmt);
    }
    ...
    function withdrawAssets(IERC20 token, address recipient, uint256 amount) external virtual onlyOwner {
        if (token == debtToken) {
            ITermMaxMarketV1Plus(address(market)).burn(address(this), recipient, amount);
        } else {
