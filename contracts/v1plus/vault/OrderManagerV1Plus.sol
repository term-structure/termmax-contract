...
contract OrderManagerV1Plus is VaultStorageV1Plus, VaultErrors, VaultEvents, IOrderManager {
    ...
    function updateOrders(
        ...
        if (length != changes.length || length != maxSupplies.length || length != curveCuts.length) {
            revert VaultErrorsV1Plus.ArrayLengthMismatch();
        }
    ...
    function _checkIdleFundRate(IERC20 asset) internal view {
        ...
        if (__minIdleFundRate > 0) {
            ...
            if (currentIdleFundRate < __minIdleFundRate) {
                revert VaultErrorsV1Plus.IdleFundRateTooLow(currentIdleFundRate, __minIdleFundRate);
            }
        }
    }
    ...
    function _accruedInterest() internal {
        ...
        emit VaultEventsV1Plus.AccruedInterest(_accretingPrincipal, _performanceFee);
    ...
    function _checkApy() internal view {
        ...
        if (__minAPy > 0) {
            ...
            if (currentApy < __minAPy) {
                revert VaultErrorsV1Plus.ApyTooLow(currentApy, __minAPy);
            }
        }
    }
