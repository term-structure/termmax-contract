...
import {VaultEventsV1Plus} from "../events/VaultEventsV1Plus.sol";
import {IOrderManager} from "../../v1/vault/IOrderManager.sol";
import {VaultStorageV1Plus, OrderInfo} from "./VaultStorageV1Plus.sol";
import {Constants} from "../../v1/lib/Constants.sol";
import {ITermMaxVault} from "../../v1/vault/ITermMaxVault.sol";
import {ITermMaxVaultV1Plus} from "./ITermMaxVaultV1Plus.sol";
import {VaultErrorsV1Plus} from "../errors/VaultErrorsV1Plus.sol";
import {TransactionReentrancyGuard} from "../lib/TransactionReentrancyGuard.sol";

contract TermMaxVaultV1Plus is
    VaultStorageV1Plus,
    ITermMaxVault,
    ...
    TransactionReentrancyGuard
{
    ...
    function initialize(VaultInitialParamsV1Plus memory params) external virtual initializer {
    ...
    function initialize(VaultInitialParams memory) external virtual initializer {
        revert VaultErrorsV1Plus.UseVaultInitialParamsV1Plus();
    }
    ...
    function supplyQueue(uint256) external view virtual returns (address) {
        revert VaultErrorsV1Plus.SupplyQueueNoLongerSupported();
    }
    ...
    function apr() external view virtual returns (uint256) {
        revert VaultErrorsV1Plus.UseApyInsteadOfApr();
    }

    /**
     * @inheritdoc ITermMaxVaultV1Plus
     */
    ...
    function submitPendingMinApy(uint64 newMinApy) external virtual override onlyCuratorRole {
        ...
        if (newMinApy > _minApy) {
            ...
            emit VaultEventsV1Plus.SubmitMinApy(newMinApy, _pendingMinApy.validAt);
        }
    }
    ...
    function submitPendingMinIdleFundRate(uint64 newMinIdleFundRate) external virtual override onlyCuratorRole {
        ...
        if (newMinIdleFundRate > _minIdleFundRate) {
            ...
            emit VaultEventsV1Plus.SubmitMinIdleFundRate(newMinIdleFundRate, _pendingMinIdleFundRate.validAt);
        }
    }
    ...
    function _setMinApy(uint64 newMinApy) internal {
        _minApy = newMinApy;
        emit VaultEventsV1Plus.SetMinApy(_msgSender(), newMinApy);
    }
    ...
    function _setMinIdleFundRate(uint64 newMinIdleFundRate) internal {
        ...
        emit VaultEventsV1Plus.SetMinIdleFundRate(_msgSender(), newMinIdleFundRate);
    }
    ...
    {
        if (collateral == asset()) revert VaultErrorsV1Plus.CollateralIsAsset();
    ...
    function revokePendingMinApy() external virtual onlyGuardianRole {
        ...
        emit VaultEventsV1Plus.RevokePendingMinApy(_msgSender());
    }
    ...
    function revokePendingMinIdleFundRate() external virtual onlyGuardianRole {
        ...
        emit VaultEventsV1Plus.RevokePendingMinIdleFundRate(_msgSender());
    }
