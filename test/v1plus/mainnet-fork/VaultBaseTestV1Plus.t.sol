...
abstract contract VaultBaseTestV1Plus is ForkBaseTestV1Plus {
    using SafeCast for *;
    ...
    struct VaultTestRes {
        ...
        ITermMaxOrder order;
        ITermMaxVault vault;
        VaultInitialParamsV1Plus vaultInitialParams;
        uint256 currentTime;
        uint256 maxCapacity;
        address maker;
    }
    ...
    function _initializeVaultTestRes(string memory key) internal returns (VaultTestRes memory) {
        ...
        res.oracle.submitPendingOracle(
            ...
            IOracleV1Plus.Oracle(res.collateralPriceFeed, res.collateralPriceFeed, 365 days, 365 days, 0, 0)
        );
        res.oracle.submitPendingOracle(
            address(res.marketInitialParams.debtToken),
            IOracleV1Plus.Oracle(res.debtPriceFeed, res.debtPriceFeed, 365 days, 365 days, 0, 0)
        );
