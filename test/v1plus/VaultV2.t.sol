...
import {ITermMaxOrder, TermMaxOrderV1Plus, ISwapCallback, OrderEvents} from "contracts/v1plus/TermMaxOrderV1Plus.sol";
...
contract VaultTestV1Plus is Test {
    ...
    VaultInitialParamsV1Plus initialParams;
    ...
    function setUp() public {
        ...
        initialParams = VaultInitialParamsV1Plus(
            deployer,
            curator,
            guardian,
            timelock,
    ...
    function testSupplyQueue() public {
        vm.expectRevert(VaultErrorsV1Plus.SupplyQueueNoLongerSupported.selector);
    ...
    function testWithdrawQueue() public {
        vm.expectRevert(VaultErrorsV1Plus.WithdrawalQueueNoLongerSupported.selector);
    ...
    function testActions() public {
        ...
        console.log("apy:", ITermMaxVaultV1Plus(address(vault)).apy());
        ...
        console.log("1-apy:", ITermMaxVaultV1Plus(address(vault)).apy());
        swapFtToXt(94.247e8, 2000e8);
        console.log("2-principal after swap:", vault.totalAssets());
        console.log("2-anulizedInterest:", vault.annualizedInterest());
        console.log("2-apy:", ITermMaxVaultV1Plus(address(vault)).apy());
    ...
    function testDealBadDebtRevert() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(VaultErrorsV1Plus.CollateralIsAsset.selector));
    ...
    // ========== Tests for ITermMaxVaultV1Plus new functions ==========

    function testMinApyManagement() public {
        ITermMaxVaultV1Plus vaultV1Plus = ITermMaxVaultV1Plus(address(vault));

        // Test initial state
        assertEq(vaultV1Plus.minApy(), 0);
        assertEq(vaultV1Plus.pendingMinApy().value, 0);
        assertEq(vaultV1Plus.pendingMinApy().validAt, 0);

        // Test submitPendingMinApy - increase (immediate)
        uint64 newMinApy = 0.05e8; // 5%
        vm.prank(curator);
        vaultV1Plus.submitPendingMinApy(newMinApy);

        // Should be set immediately for increases
        assertEq(vaultV1Plus.minApy(), newMinApy);
        assertEq(vaultV1Plus.pendingMinApy().value, 0);

        // Test submitPendingMinApy - decrease (timelock required)
        uint64 lowerMinApy = 0.03e8; // 3%
        vm.prank(curator);
        vaultV1Plus.submitPendingMinApy(lowerMinApy);

        // Should not be set immediately for decreases
        assertEq(vaultV1Plus.minApy(), newMinApy);
        assertEq(vaultV1Plus.pendingMinApy().value, lowerMinApy);
        assertEq(vaultV1Plus.pendingMinApy().validAt, block.timestamp + timelock);

        // Test acceptPendingMinApy before timelock
        vm.expectRevert(VaultErrors.TimelockNotElapsed.selector);
        vaultV1Plus.acceptPendingMinApy();

        // Test acceptPendingMinApy after timelock
        vm.warp(block.timestamp + timelock + 1);
        vaultV1Plus.acceptPendingMinApy();

        assertEq(vaultV1Plus.minApy(), lowerMinApy);
        assertEq(vaultV1Plus.pendingMinApy().value, 0);
        assertEq(vaultV1Plus.pendingMinApy().validAt, 0);
    }
    ...
    function testMinApyRevoke() public {
        ...
        vm.prank(curator);
        vaultV1Plus.submitPendingMinApy(initialMinApy);
        ...
        vm.prank(guardian);
        vaultV1Plus.revokePendingMinApy();

        assertEq(vaultV1Plus.minApy(), initialMinApy);
        assertEq(vaultV1Plus.pendingMinApy().value, 0);
        assertEq(vaultV1Plus.pendingMinApy().validAt, 0);
    }
    ...
    function testMinIdleFundRateManagement() public {
        ...
        // Test initial state
        assertEq(vaultV1Plus.minIdleFundRate(), 0);
        assertEq(vaultV1Plus.pendingMinIdleFundRate().value, 0);
        assertEq(vaultV1Plus.pendingMinIdleFundRate().validAt, 0);

        // Test submitPendingMinIdleFundRate - increase (immediate)
        uint64 newMinIdleFundRate = 0.1e8; // 10%
        vm.prank(curator);
        vaultV1Plus.submitPendingMinIdleFundRate(newMinIdleFundRate);

        // Should be set immediately for increases
        assertEq(vaultV1Plus.minIdleFundRate(), newMinIdleFundRate);
        assertEq(vaultV1Plus.pendingMinIdleFundRate().value, 0);

        // Test submitPendingMinIdleFundRate - decrease (timelock required)
        uint64 lowerMinIdleFundRate = 0.05e8; // 5%
        vm.prank(curator);
        vaultV1Plus.submitPendingMinIdleFundRate(lowerMinIdleFundRate);

        // Should not be set immediately for decreases
        assertEq(vaultV1Plus.minIdleFundRate(), newMinIdleFundRate);
        assertEq(vaultV1Plus.pendingMinIdleFundRate().value, lowerMinIdleFundRate);
        assertEq(vaultV1Plus.pendingMinIdleFundRate().validAt, block.timestamp + timelock);

        // Test acceptPendingMinIdleFundRate before timelock
        vm.expectRevert(VaultErrors.TimelockNotElapsed.selector);
        vaultV1Plus.acceptPendingMinIdleFundRate();

        // Test acceptPendingMinIdleFundRate after timelock
        vm.warp(block.timestamp + timelock + 1);
        vaultV1Plus.acceptPendingMinIdleFundRate();

        assertEq(vaultV1Plus.minIdleFundRate(), lowerMinIdleFundRate);
        assertEq(vaultV1Plus.pendingMinIdleFundRate().value, 0);
        assertEq(vaultV1Plus.pendingMinIdleFundRate().validAt, 0);
    }
    ...
    function testMinIdleFundRateRevoke() public {
        ...
        vm.prank(curator);
        vaultV1Plus.submitPendingMinIdleFundRate(initialMinIdleFundRate);
        ...
        // Guardian revokes pending change
        vm.prank(guardian);
        vaultV1Plus.revokePendingMinIdleFundRate();

        assertEq(vaultV1Plus.minIdleFundRate(), initialMinIdleFundRate);
        assertEq(vaultV1Plus.pendingMinIdleFundRate().value, 0);
        assertEq(vaultV1Plus.pendingMinIdleFundRate().validAt, 0);
    }
    ...
    function test_RevertMinApyAccessControl() public {
        ...
        vm.prank(vm.randomAddress());
        vm.expectRevert(VaultErrors.NotCuratorRole.selector);
        vaultV1Plus.submitPendingMinApy(0.05e8);
    ...
    function test_RevertMinIdleFundRateAccessControl() public {
        ...
        vm.prank(vm.randomAddress());
        vm.expectRevert(VaultErrors.NotCuratorRole.selector);
        vaultV1Plus.submitPendingMinIdleFundRate(0.1e8);
    ...
    function test_RevertMinApyAlreadySet() public {
        ...
        vm.startPrank(curator);
        vaultV1Plus.submitPendingMinApy(minApy);
    ...
    function test_RevertMinIdleFundRateAlreadySet() public {
        ...
        vm.startPrank(curator);
        vaultV1Plus.submitPendingMinIdleFundRate(minIdleFundRate);
    ...
    function test_RevertMinApyAlreadyPending() public {
        ...
        vaultV1Plus.submitPendingMinApy(0.03e8);

        // Should revert when trying to submit another pending change
        vm.expectRevert(VaultErrors.AlreadyPending.selector);
        vaultV1Plus.submitPendingMinApy(0.02e8);
    ...
    function test_RevertMinIdleFundRateAlreadyPending() public {
        ...
        // Submit decrease (creates pending)
        vaultV1Plus.submitPendingMinIdleFundRate(0.05e8);

        // Should revert when trying to submit another pending change
        vm.expectRevert(VaultErrors.AlreadyPending.selector);
        vaultV1Plus.submitPendingMinIdleFundRate(0.03e8);
    ...
    function testMinApyEventEmission() public {
        ...
        emit VaultEventsV1Plus.SetMinApy(curator, newMinApy);
        ...
        emit VaultEventsV1Plus.SubmitMinApy(lowerMinApy, uint64(block.timestamp + timelock));
        ...
        vm.warp(block.timestamp + timelock + 1);
        vm.expectEmit(true, false, false, true);
        emit VaultEventsV1Plus.SetMinApy(address(this), lowerMinApy);
    ...
    function testMinIdleFundRateEventEmission() public {
        ...
        vm.expectEmit(true, false, false, true);
        emit VaultEventsV1Plus.SetMinIdleFundRate(curator, newMinIdleFundRate);
        ...
        uint64 lowerMinIdleFundRate = 0.05e8;
        vm.expectEmit(false, false, false, true);
        emit VaultEventsV1Plus.SubmitMinIdleFundRate(lowerMinIdleFundRate, uint64(block.timestamp + timelock));
        ...
        vm.warp(block.timestamp + timelock + 1);
        vm.expectEmit(true, false, false, true);
        emit VaultEventsV1Plus.SetMinIdleFundRate(address(this), lowerMinIdleFundRate);
    ...
    function testRevokeEventEmission() public {
        ...
        vm.startPrank(curator);
        vaultV1Plus.submitPendingMinApy(0.05e8);
        vaultV1Plus.submitPendingMinApy(0.03e8); // Creates pending
        vaultV1Plus.submitPendingMinIdleFundRate(0.1e8);
        vaultV1Plus.submitPendingMinIdleFundRate(0.05e8); // Creates pending
        vm.stopPrank();

        // Test RevokePendingMinApy event
        vm.expectEmit(true, false, false, false);
        emit VaultEventsV1Plus.RevokePendingMinApy(guardian);
        ...
        vm.expectEmit(true, false, false, false);
        emit VaultEventsV1Plus.RevokePendingMinIdleFundRate(guardian);
    ...
    function testRevertWhenIdleFundTooLow() public {
        ITermMaxVaultV1Plus vaultV1Plus = ITermMaxVaultV1Plus(address(vault));
        vm.startPrank(curator);
        vaultV1Plus.submitPendingMinIdleFundRate(0.1e8); // minIdleFundRate = 10%
        ...
        vm.expectRevert(abi.encodeWithSelector(VaultErrorsV1Plus.IdleFundRateTooLow.selector, 0, 0.1e8));
        vault.updateOrders(orders, amounts, maxSupplies, curveCuts);
    ...
    function testRevertWhenApyTooLow() public {
        ITermMaxVaultV1Plus vaultV1Plus = ITermMaxVaultV1Plus(address(vault));
        vm.prank(curator);
        vaultV1Plus.submitPendingMinApy(0.05e8); // minApy = 5%
        ...
        vm.expectRevert(abi.encodeWithSelector(VaultErrorsV1Plus.ApyTooLow.selector, 0, 0.05e8));
        res.order.swapExactTokenToToken(res.debt, res.ft, taker, tokenAmtIn, ftAmtOut, block.timestamp + 1 hours);
        vm.stopPrank();
    }

    // ========== Tests for ITermMaxVaultV1Plus nonTxReentrantBetweenActions ==========
